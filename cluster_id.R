...
## see example data for expected format

cluster.data <- fread("my_cluster_data.csv", check.names = FALSE)

# remove any unwanted columns
## you will need to edit this command to remove any non-marker columns in your data
input.data <- cluster.data
# input.data <- input.data %>% select(-c(OmiqFilter, OmiqFileIndex))
...
cell.type.options <- unique( cell.database$Parent.cell.type )

# punch in your cell type or types
selected.cell.type <- c("B cell")
...
fwrite(cluster.data, file = "labeled_cluster_data_simple.csv")
...
# set cluster colors
n.clusters <- length(unique(input.data$Cluster))
color.palette <- viridis(n.clusters)
names(color.palette) <- unique(input.data$Cluster)
input.data$Cluster_label <- cluster.data$Cluster_label
input.data$Clust_color <- color.palette[input.data$Cluster]
cluster.data$Clust_color <- input.data$Clust_color

input.data.long <- input.data %>%
  pivot_longer( -c(Cluster, Cluster_label, Clust_color), names_to = "Marker", 
                values_to = "Expression")

ggplot(input.data.long, aes(x = Expression, y = Marker, fill = Clust_color)) +
...
for (cluster.group in non.unique.groups) {
  ...
  data.temp.2 <- data.temp %>% select(-Cluster, -Cluster_label)
  # find most variable channels
  variances <- apply(data.temp.2, 2, var)
  sorted.variances <- sort(variances, decreasing = TRUE)
  
  # find min and max (neg and pos) for each channel
  max.expression <- apply(data.temp.2, 2, max)
  min.expression <- apply(data.temp.2, 2, min)
  
  for( cluster in 1:nrow(data.temp)){
    # get position of cluster in full list
    cluster.name <- data.temp$Cluster_label[cluster]
    
    cluster.list.idx <- grep(names(cluster.name), names(cluster.summary$Cluster_label))
    #position.in.cluster.list <- cluster.summary$Cluster_label[
     # grepl(cluster.group, cluster.summary$Cluster_label)][cluster.list.idx]
    ...
    for( marker in names(sorted.variances)[1:markers.to.append.n]){
      cluster.summary$Cluster_label[cluster.list.idx] <- 
        if ( abs(cluster.summary[cluster.list.idx,marker] - max.expression[marker] ) < abs(cluster.summary[cluster.list.idx,marker] - min.expression[marker] )){
          paste( cluster.summary$Cluster_label[cluster.list.idx], marker )
        } else{
          cluster.summary$Cluster_label[cluster.list.idx] <- cluster.summary$Cluster_label[cluster.list.idx]
        }
    }
  }
}
...
cluster.data.long <- cluster.data %>%
  # remove any unwanted columns
  # select(-OmiqFilter, -OmiqFileIndex) %>%
